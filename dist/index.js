...
function getNormalizedFileName(fileName, includeExt = true) {
    let { dir, name, ext } = path__default["default"].parse(vite.normalizePath(path__default["default"].normalize(fileName)));
    if (!dir) {
...
const addHmrSupportToCsp = (hmrServerOrigin, inlineScriptHashes, contentSecurityPolicyStr) => {
    ...
    const contentSecurityPolicy = parse__default["default"](contentSecurityPolicyStr || "");
...
class DevBuilder {
    ...
    async writeBuild({ devServerPort, manifest, manifestHtmlFiles, }) {
        ...
        fsExtra.copy("public", this.outDir);
        ...
        await fsExtra.writeFile(`${this.outDir}/manifest.json`, JSON.stringify(manifest, null, 2));
    }
    ...
    async writeManifestHtmlFile(fileName, absoluteFileName) {
        ...
        const inputFileDir = path__default["default"].dirname(fileName);
        ...
        const outFileDir = path__default["default"].dirname(outFile);
...
class DevBuilderManifestV2 extends DevBuilder {
    ...
    parseInlineScriptHashes(content) {
        ...
        for (const match of matches) {
            const shasum = crypto__default["default"].createHash("sha256");
        ...
        (_a = result.manifest.content_scripts) === null || _a === void 0 ? void 0 : _a.forEach((script) => {
            ...
            (_b = script.css) === null || _b === void 0 ? void 0 : _b.forEach((cssFile) => {
                result.emitFiles.push({
                    type: "asset",
                    fileName: cssFile,
                    source: fsExtra.readFileSync(cssFile, "utf-8"),
                });
            });
        });
        return result;
    ...
    res.end = function end(chunk, ...otherArgs) {
        if (req.url === "/@vite/client" && typeof chunk === "string") {
            if (!/const sheetsMap/.test(chunk) ||
                ...
                res.setHeader("Etag", getEtag__default["default"](chunk, { weak: true }));
                // @ts-ignore
...
function transformSelfLocationAssets(code, resolvedViteConfig) {
    if (code.includes("new URL") && code.includes(`self.location`)) {
        ...
        while ((match = selfLocationUrlPattern.exec(code))) {
            ...
            if (!updatedCode)
                updatedCode = new MagicString__default["default"](code);
...
function webExtension(pluginOptions) {
    ...
    let webAccessibleScriptsFilter = vite.createFilter((webConfig === null || webConfig === void 0 ? void 0 : webConfig.include) || /\.([cem]?js|ts)$/, (webConfig === null || webConfig === void 0 ? void 0 : webConfig.exclude) || "", webConfig === null || webConfig === void 0 ? void 0 : webConfig.options);
    return {
        name: "webExtension",
        enforce: "post",
        config(config) {
